...
but you also don't need [`Match`](match-details.md) details or
any replacement logic, really (so `callback()` is a little too much).

Replacing by a map is an acceptable compromise, which allows T-Regx to save a little performance
overhead, by not creating [`Match`](match-details.md) object, as well as making your code a
little more concise.
