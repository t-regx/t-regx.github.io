...
Because `flatMapAssoc()` receives an array from its callback, it's possible to also return an associative array with
specified keys:

<CodeTabs
    tregx={`pattern("\\w+")->match("Apples are cool")->flatMapAssoc(function (Detail $detail) {
...
:::note
Keep in mind, that `flatMap()` uses [`array_merge()`] to flatten the results! So any keys returned will be
reindexed.
:::
