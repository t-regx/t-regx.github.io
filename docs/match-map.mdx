...
they have been iterated (and potentially altered) using `map()` callback (which accepts [`Match`] details).
...
<CodeTabs
    tregx={`pattern("[\\w']+")->match("I'm 19 years old")->map(function (Match $match) {
