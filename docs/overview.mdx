...
- Removing headaches of PHP:
  - `if()`ing around `preg_` methods, to unify PHP cases with exceptions (see ["Comparison"](/comparison))
...
- Cleaning the [mess after PHP regular expressions API](whats-the-point.md):
  ...
  - Special values like `null`, `false`, `''` aren't used to indicate errors (exceptions instead)
  - Handles every warning, error, fatal error or notice aren't thrown (exceptions instead)
  - `preg_last_error()` isn't required to handle errors (exceptions instead)
  - Errors/traps aren't silently ignored (exceptions instead)
...
- Being explicit and descriptive - (see [why is explicit interface so important?](#why-is-explicit-interface-so-important)):
...
T-Regx' aim is to relieve programmers from [**brain strain**] while reading and writing code.
...
<CodeTabs
    tregx="$result = pattern($pattern)->match($subject)->first();"
